    ...
    V: SnarlViewer<T>,
{
    ...
    if snarl_resp.clicked() {
        viewer.background_click(snarl_resp.rect.clone(), snarl)
    }
    ...
    let mut hovered_wire_select = false;
        ...
        if !snarl_state.has_new_wires() && snarl_resp.contains_pointer() && hovered_wire.is_none() {
            ...
            if let Some(latest_pos) = latest_pos {
                ...
                if wire_hit {
                    ...
                    hovered_wire_select |= wire_r.clicked_by(PointerButton::Primary);
    ...
    // Remove hovered wire by second click
    if hovered_wire_select && let Some(wire) = hovered_wire {
        let out_pin = OutPin::new(snarl, wire.out_pin);
        let in_pin = InPin::new(snarl, wire.in_pin);
        viewer.wire_select(&out_pin, &in_pin, snarl);
    }
